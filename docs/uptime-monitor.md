...
> 推送**（2026-08-16 起掃描節奏為每 ~2 分鐘一次：cron 每分鐘觸發，但 worker
> 以 DB 心跳做 90 秒間隔閘門，跳過過早的 tick 以減輕上游 429 壓力）。cron
> 修好後，監控變成雙保險（cron + 監控都驅動掃描；掃描器有 running 鎖保護，
> 重疊呼叫只會跳過，不會重複推送）。
...
- 每次 `/health` 請求 → Worker 背景跑一次完整掃描（waitUntil 保持 isolate
  存活直到掃描完成）→ 寫入心跳。
- 外部監控每 1 分鐘 ping → 掃描維持在 ~2 分鐘一次（cron 的 90s 間隔閘門
  會跳過過早的 tick；監控 ping 只是補充驅動，不會加速超過閘門）。
- 掃描器有 running 鎖（25s 時效自動打破）+ SCAN_INTERVAL_SECONDS=90 的
