...
>
> **設一個外部監控每 1 分鐘 ping `/health`，bot 就能不依賴 cron 每分鐘正常
> 掃描推送。** cron 修好後，監控變成雙保險（cron + 監控都驅動掃描；掃描器
> 有 running 鎖保護，重疊呼叫只會跳過，不會重複推送）。
...
- 每次 `/health` 請求 → Worker 背景跑一次完整掃描（waitUntil 保持 isolate
  存活直到掃描完成）→ 寫入心跳。
- 外部監控每 1 分鐘 ping → 掃描每 1 分鐘一次 → 行為與 cron 正常時一致。
- 掃描器有 running 鎖（25s 時效自動打破）+ 60s 觸發間隔保護，cron 與監控
  同時驅動也不會重疊或重複推送。
